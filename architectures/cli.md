...
- session start/status/output/stop
- shell/background/timeout/program/cwd/env policy
...
- `CliExecutionRequest::Command(CmdRequest)`
- `CliExecutionRequest::Shell(ShellCmdRequest)`
- `CliExecutionResult`
...
```text
exit_code != 0 -> CmdOutput { status: Failed(code), stderr, stdout }
```
...
### 7. Unix 已有 process group cleanup，Windows 仍是 direct child

Unix 平台上，执行器会把 child 放入独立进程组。timeout 和 session stop 会先尝试杀进程组，再杀直接 child。
...
第二步：给 session 增加显式 cleanup/forget API，避免长期保留已结束任务。

第三步：增加 session stdout/stderr 增量 cursor。

第四步：补 write stdin 到 session。
